...
api_key="AIzaSyCd21fAMgLYvH-La6VBT2xk1n1WuDmwtws"

genai.configure(api_key=api_key)

def generate_response( prompt :str, temperature:float =0.3)-> str:
    try:
        system_prompt="You are an Math mastermind! , skilled in algebra,calculas,probability, surface&area, graphs,promblems,sums,numbers theory etc.Always provide a step-by-step solution with detailed and clear explanation with final answer!"
...
def setup_ui():
    ...
    col_clear, col_expert=st.columns(2)
    with col_clear:
    ...
    with col_expert:
        if st.session_state.history:
            text="\n\n ".join(f"A{idx+1}: {qa['question']} \n A{idx+1} : {qa['answer']}" for idx,qa in enumerate(st.session_state.history))
            st.download_button("downnload_solution", data=text, file_name="data.txt")
    if st.session_state.history:
